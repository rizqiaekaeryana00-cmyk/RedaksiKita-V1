...
const EvaluationDesk: React.FC<EvaluationDeskProps> = ({ onBack, quizQuestions }) => {
  ...
  const getPredikat = () => {
    const total = (score / (quizQuestions.length * 10)) * 100;
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            <div className="bg-slate-50 border-2 border-black p-4 rounded-2xl shadow-[4px_4px_0px_#000]">
              ...
              <p className="text-4xl font-black text-[#00E5FF] leading-none">{Math.round((score / (quizQuestions.length * 10)) * 100)}%</p>
            </div>
          </div>
          <button onClick={onBack} className="group w-full py-4 bg-[#FF3D00] text-white border-4 border-black rounded-2xl font-black uppercase tracking-widest text-lg hover:scale-102 transition-all shadow-[6px_6px_0px_#000] active:translate-y-1 active:shadow-none flex items-center justify-center space-x-3">
  ...
  if (!current) return <div className="p-20 text-center font-black">BELUM ADA SOAL KUIS.</div>;
      ...
      <div className="w-full max-w-4xl flex-1 flex flex-col justify-between">
        <div className="text-center px-4">
           <div className="h-3 w-full bg-slate-200 rounded-full border-2 border-black overflow-hidden mb-4 shadow-sm">
              <motion.div initial={{ width: 0 }} animate={{ width: `${((currentIdx + 1) / quizQuestions.length) * 100}%` }} className="h-full bg-[#4CAF50]"></motion.div>
           </div>
           <motion.div key={currentIdx} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-4">
             <h3 className="text-xl md:text-2xl font-black leading-tight text-black italic tracking-tighter">"{current.question}"</h3>
           </motion.div>
        </div>
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 gap-4 mb-4`}>
          {current.options.map((opt, idx) => {
            ...
            return (
              ...
              >
                <div className={`shrink-0 w-10 h-10 rounded-lg flex items-center justify-center font-black text-xl border-2 border-black mr-4 ${showCorrect ? 'bg-green-500 text-white' : showWrong ? 'bg-red-500 text-white' : 'bg-slate-100 text-black'}`}>
                  {String.fromCharCode(65 + idx)}
                </div>
                <span className="text-sm md:text-base font-black text-black tracking-tight leading-snug flex-1">{opt}</span>
              </motion.button>
            );
          })}
        </div>
        ...
        <div className="min-h-[140px] flex items-end">
          <AnimatePresence>
            {showFeedback && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={`w-full p-4 rounded-2xl border-4 border-black shadow-[8px_8px_0px_#000] relative overflow-hidden ${selectedOpt === current.correctAnswer ? 'bg-green-50' : 'bg-red-50'}`}>
                <div className="flex items-start gap-4">
                   <div className="flex-1">
                      <p className="text-lg font-black italic text-black mb-2 leading-tight tracking-tight">"{loadingMsg ? 'Sedang mengetik...' : editorMsg}"</p>
                   ...
                   <motion.button onClick={nextQuestion} className="self-center bg-black text-white p-4 rounded-xl hover:bg-slate-900 transition-all border-2 border-white shadow-lg group">
                    <ArrowRight className="w-6 h-6 group-hover:translate-x-1 transition-transform" />
                  </motion.button>
                </div>
              </motion.div>
